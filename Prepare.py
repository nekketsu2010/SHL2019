...
for num in range(len(folders)):
    ...
    for file in files:
        ...
        for x_acc, y_acc, z_acc, x_gra, y_gra, z_gra, x_mag, y_mag, z_mag in zip(X_Acc, Y_Acc, Z_Acc, X_Gra, Y_Gra, Z_Gra, X_Mag, Y_Mag, Z_Mag):
            inR = [0] * 16
        ...
        with open(directoryName + folderName + ".csv", mode="a") as newcsv:
            newcsv.write(str((num+1)) + ',' + str(出XY_accel) + ',' + str(出XY_accel_variance) + ',' + str(出Z_accel) + ',' + str(出Z_accel_variance) + '\n')
